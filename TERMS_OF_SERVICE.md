# Terms of Service

**Last Updated: November 24, 2025**

## Welcome to MISAKI Bot

By inviting and using MISAKI Bot ("the Bot"), you agree to comply with and be bound by the following Terms of Service. Please review these terms carefully.

## 1. Acceptance of Terms

By accessing or using MISAKI Bot, you agree to be bound by these Terms of Service and all applicable laws and regulations. If you do not agree with any of these terms, you are prohibited from using the Bot.

## 2. Use License

MISAKI Bot is provided for entertainment and utility purposes. You are granted a limited, non-exclusive, non-transferable license to use the Bot for personal or community server purposes.

### You MAY:
- Use the Bot in multiple Discord servers
- Configure server-specific settings (prefix, 24/7 mode, etc.)
- Use all music playback features
- Share the Bot invite link with others

### You MAY NOT:
- Attempt to reverse engineer or decompile the Bot
- Use the Bot for illegal activities
- Abuse or spam Bot commands
- Use the Bot to violate Discord's Terms of Service
- Attempt to gain unauthorized access to the Bot's systems

## 3. Music Playback

MISAKI Bot provides music streaming functionality from various platforms:
- You must have the legal right to play requested content
- The Bot does not store or distribute copyrighted content
- Music is streamed in real-time from third-party sources
- We are not responsible for the availability of third-party music sources

## 4. Data Collection

Please refer to our [Privacy Policy](PRIVACY_POLICY.md) for information on data collection and usage.

## 5. Service Availability

- MISAKI Bot is provided "as is" without warranties
- We do not guarantee 100% uptime or availability
- The Bot may undergo maintenance or updates without notice
- Features may be added, modified, or removed at any time

## 6. User Conduct

Users must:
- Respect other users and server members
- Not use the Bot to harass, abuse, or harm others
- Not attempt to crash, lag, or overload the Bot
- Report bugs or issues responsibly

## 7. Limitation of Liability

MISAKI Bot and its developers:
- Are not liable for any damages resulting from Bot usage
- Are not responsible for content played through the Bot
- Are not liable for server disruptions or data loss
- Reserve the right to terminate service to any user or server

## 8. Server Administrator Rights

Server administrators have the right to:
- Configure Bot settings for their server
- Change command prefixes
- Enable/disable 24/7 mode
- Remove the Bot from their server at any time

## 9. Changes to Terms

We reserve the right to modify these Terms of Service at any time. Continued use of the Bot after changes constitutes acceptance of the new terms.

## 10. Termination

We reserve the right to:
- Terminate or suspend access to the Bot without notice
- Remove the Bot from servers violating these terms
- Ban users or servers from using the Bot

## 11. Third-Party Services

MISAKI Bot integrates with third-party services (Discord, Wavelink, music sources). We are not responsible for:
- Third-party service outages
- Changes to third-party APIs
- Third-party terms of service violations

## 12. Contact

For questions about these Terms of Service, please contact the Bot developer through Discord.

## 13. Governing Law

These Terms shall be governed by and construed in accordance with applicable laws, without regard to conflicts of law provisions.

---

**By using MISAKI Bot, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.**

